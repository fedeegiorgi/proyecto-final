...
cdef class AddOnBestSplitter(Splitter):
    ...
    cdef int init_addon(
        ...
        self.rand_r_state = self.random_state.randint(0, RAND_R_MAX)
        cdef intp_t n_samples = X_original.shape[0]
        ...
        # Count of features with original data 
        cdef intp_t n_features = X_original.shape[1]
        self.features = np.arange(n_features, dtype=np.intp)
        self.n_features = n_features

        # Count of features with peer prediction data
        cdef intp_t n_features_pp = X_peer_prediction.shape[1]
        self.features_pp = np.arange(n_features_pp, dtype=np.intp)
        self.n_features_pp = n_features_pp
        ...
        # Max to initial features
        self.max_features = n_features

        self.partitioner = DensePartitioner(
            X_peer_prediction, self.samples, self.feature_values, missing_values_in_feature_mask
        )
