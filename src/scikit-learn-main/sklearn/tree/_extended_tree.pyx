...
cdef class DepthFirstTreeExtensionBuilder(TreeBuilder):
    ...
    cpdef build_extended_2(
        ...
        splitter.init_addon(X_original, X_peer_prediction, initial_max_depth, y, sample_weight, missing_values_in_feature_mask)
            ...
            while not builder_stack.empty():
                ...
                with gil:
                    if split.feature > 205 and not is_leaf:
                        print(f"!!!!!!!!!!!!!!! Use feature amiga N° {split.feature} en nodo {node_id}, depth: {depth} !!!!!!!!!!!!!!!")
        ...
        for i in range(initial_n_nodes):
            if not self.is_leafs[i]:
                ...
                if not self.is_lefts[i]:
                    ...
                    print("Impurezas:", self.impurities)
                    self.builder_stack.push({
                        "start": split_pos[self.parents[i]],
                        "end": end_pos[self.parents[i]],
                        "depth": self.depths[i],
